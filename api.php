<?php 
    ...
    else if($get_method['method_name'] == "admission_inquiry")
    {
        ...
        {
            //------------------ CLASS ------------------------
            $sqllms	= $dblms->querylms("SELECT class_id  
                                            FROM ".CLASSES."  
                                            WHERE class_id != '' AND class_name LIKE '%".cleanvars($get_method['class'])."%' LIMIT 1");
            $rowsvalue = mysqli_fetch_array($sqllms);
            //-------------------------------------------------
            	
            //-------------------- CAMPUS ---------------------
            $sqllmsCampus  = $dblms->querylms("SELECT campus_code
                                                FROM ".CAMPUS." 
                                                WHERE campus_status = '1' AND campus_id = '".cleanvars($get_method['campus'])."' LIMIT 1");
            $value_campus = mysqli_fetch_array($sqllmsCampus);
            //--------------------------------------------------

            //-------------------- GET FROM NO -----------------
            $sqllmsInq	= $dblms->querylms("SELECT MAX(id) as form
                                            FROM ".ADMISSIONS_INQUIRY." 
                                            WHERE id_campus = '".cleanvars($get_method['campus'])."' ");
            $valueInq = mysqli_fetch_array($sqllmsInq);
            $maxNo = $valueInq['form'] + 1;
            //--------------------------------------------------
            $form_no = $value_campus['campus_code'].'-'.$maxNo;
            //------------- ADMISSION INQUIRY -------------------
            $sqllmsInsert  = $dblms->querylms("INSERT INTO ".ADMISSIONS_INQUIRY."(
                                                                ...
                                                                VALUES(
                                                                        ...
                                                                        '".cleanvars($rowsvalue['class_id'])."'	    ,
                                                                        '".cleanvars($get_method['campus'])."'		,
                                                                        NOW()
                                                                    )"
                                            );
            //------------------ GENRATE NOTIFICATION --------------
            $not_title = "Admission Inquiry for: ".$form_no."";
            $not_desc  = "
                            Admission Inquiry for: ".$form_no.",
                            Class ".$get_method['class'].",
                            Student: ".$get_method['name']." 
                            ".$get_method['note']."
                        ";
            $dated = date('Y-m-d');
            //------------------------------------
    ...
    else if($get_method['method_name'] == "subjects") 
    {
        ...
        if($response['login_for'] == 3)
        {
            ...
            $sqllms_subject	= $dblms->querylms("SELECT t.id_session, t.id_class, t.id_section, s.subject_id, s.subject_code, s.subject_name
                                                        ...
                                                        AND id_session = '".$response['id_session']."'
                                                        AND t.status = '1' GROUP BY s.subject_id
                                                    ");
    ...
    else if($get_method['method_name'] == "syllabus_breakdown") 
    {
        ...
        if($response['login_for'] == 3)
        {
            ...
            while($value_resources = mysqli_fetch_array($sqllms_resources))
            {
                ...
                $link = $url."uploads/syllabus/".$value_resources['syllabus_file'];
