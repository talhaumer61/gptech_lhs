	...
	<form action="attendance_students.php" id="form" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			...
			<div class="panel-body">';
				$sqllms	= $dblms->querylms("SELECT s.std_id, s.std_status,  s.std_name, s.std_fathername, s.std_photo,
												...
												AND s.std_status = '1' AND s.id_class = '".$class."' AND s.id_section = '".$section."'
												AND s.is_deleted != '1'");
					...
					<table class="table table-bordered table-striped table-condensed mb-none ">
						...
						<tbody>';
							...
							while($rowsvalues = mysqli_fetch_array($sqllms)) {
								$srno++;
								$std_photo = ((!empty($val['std_photo']) && file_exists('uploads/images/students/'.$rowsvalues['std_photo'])) ? 'uploads/images/students/'.$rowsvalues['std_photo'] : 'uploads/default-student.jpg');
								echo'
								<tr>
									<td>'.$srno.'</td>
									<td class="center"> <img src="'.$std_photo.'" width="35" height="35"</td>  
...
//-----------------------------------------------------
  $sqllms	= $dblms->querylms("SELECT a.id, a.status, a.dated, a.id_class, a.id_section,
								   ...
								   AND s.id_class = '".$_POST['id_class']."' AND s.id_section = '".$_POST['id_section']."' AND s.is_deleted != '1'
								   AND a.dated = '".$dated."' ");
//-----------------------------------------------------
if(mysqli_num_rows($sqllms) > 0){
echo'
