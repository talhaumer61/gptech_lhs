...
if(($_SESSION['userlogininfo']['LOGINTYPE'] == 1) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '16', 'view' => '1'))){
	...
	if(isset($_POST['show_results'])){
		// GENDER
		if($_POST['emply_gender']){
			$emply_gender = $_POST['emply_gender'];
			$sql1 = "AND e.emply_gender = '".$emply_gender."'";
		}
		// STATUS
		if($_POST['emply_status']){
			$emply_status = $_POST['emply_status'];
			$sql2 = "AND e.emply_status = '".$emply_status."'";
		}
		// SEARCH WORDS
		if($_POST['search_word']){
			$search_word = $_POST['search_word'];
			$sql3 = "AND (e.emply_name LIKE '%".$search_word."%' OR e.emply_regno LIKE '%".$search_word."%')";
		}
		// DESIGNATION
		if($_POST['id_designation']){
			$id_designation = $_POST['id_designation'];
			$sql4 = "AND e.id_designation = '".$id_designation."'";
		}
		// DEPARTMENT
		if($_POST['id_dept']){
			$id_dept = $_POST['id_dept'];
			$sql5 = "AND e.id_dept = '".$id_dept."'";
		}
	}
	echo'
				...
				<div class="row form-group mb-lg">
					<div class="col-md-3">
						...
						<div class="form-group">
							<input type="search" name="search_word" id="search_word" class="form-control" value="'.$search_word.'" placeholder="employee name or registration number" aria-controls="table_export">
						</div>
					</div>
					<div class="col-sm-2">
						<div class="form-group">
							<label class="control-label">Department</label>
							<select class="form-control" title="Must Be Required" data-plugin-selectTwo data-width="100%" name="id_dept">
								...
								while($valDept = mysqli_fetch_array($sqlDept)) {
									echo '<option value="'.$valDept['dept_id'].'" '.($id_dept==$valDept['dept_id'] ? 'selected' : '').'>'.$valDept['dept_name'].'</option>';
								}
								echo'
							</select>
						</div>
					...
					<div class="col-sm-2">
						<div class="form-group">
							...
							<select class="form-control" title="Must Be Required" data-plugin-selectTwo data-width="100%" name="id_designation">
									<option value="">Select</option>';
									...
									while($valDesignation = mysqli_fetch_array($sqlDesignation)) {
										echo '<option value="'.$valDesignation['designation_id'].'" '.($id_designation==$valDesignation['designation_id'] ? 'selected' : '').'>'.$valDesignation['designation_name'].'</option>';
									}
							echo '
								</select>
						</div>
					...
					<div class="col-md-2">
						...
						<select class="form-control" data-plugin-selectTwo data-width="100%" name="emply_gender">
							...
							foreach($gender as $gndr){
								echo '<option value="'.$gndr.'" '.($emply_gender == $gndr ? 'selected' : '').'>'.$gndr.'</option>';
							}
							echo'
						</select>
					</div>
					<div class="col-md-2">
						...
						<select class="form-control" data-plugin-selectTwo data-width="100%" name="emply_status">
							<option value="">Select</option>';
							foreach($admstatus as $stat){
								echo '<option value="'.$stat['id'].'" '.($emply_status == $stat['id'] ? 'selected' : '').'>'.$stat['name'].'</option>';
							}
							echo'
						</select>
					</div>
					<div class="col-md-1">
						<div class="form-group mt-xl">
							<button type="submit" name="show_results" class="btn btn-primary btn-block"><i class="fa fa-search"></i></button>
						</div>
					</div>
				</div>
			</form>
								...
								<div class="table-responsive">
									...
									<table width="100%" class="mt-md">
										<tbody>
											...
											<tr>
												<td class="pr-10 pl-10">Administering tests to evaluate student\'s progress.</td>
											</tr>
											<tr>
