...
$sqllms	= $dblms->querylms("SELECT a.adm_id, a.adm_status, a.adm_username, a.adm_fullname,
								a.adm_email, a.adm_phone, a.adm_photo, c.campus_id, c.principal_sign
								FROM ".ADMINS." a  
...
//-----------------------------------------------------
if($rowsvalues['principal_sign']){
	$sign = 'uploads/images/signature/principal/'.$rowsvalues['principal_sign'];
}
else{
	$sign = '';
}
...
<form action="#" class="form-horizontal validate" enctype="multipart/form-data" method="post" accept-charset="utf-8">
	...
	<input type="hidden" name="campus_id" value="'.$rowsvalues['campus_id'].'">
	<fieldset class="mt-lg">
		...
		<div class="form-group">
			<label class="col-sm-3 control-label">Principal Signature</label>
			<div class="col-md-8">
				<div class="fileinput fileinput-new" data-provides="fileinput">
					<div class="fileinput-new thumbnail" style="width: 100px; height: 100px;" data-trigger="fileinput">
							<img src="'.$sign.'" class="rounded img-responsive">
					</div>
					<div class="fileinput-preview fileinput-exists thumbnail" style="max-width: 200px; max-height: 150px"></div>
					<div>
						<span class="mr-xs btn btn-xs btn-default btn-file">
							<span class="fileinput-new">Select image</span>
							<span class="fileinput-exists">Change</span>
							<input type="file" name="principal_sign" accept="image/*">
						</span>
						<a href="#" class="btn btn-xs btn-warning fileinput-exists" data-dismiss="fileinput">Remove</a>
					</div>
				</div>
