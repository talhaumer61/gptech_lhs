<?php
if(($_SESSION['userlogininfo']['LOGINTYPE'] == 1) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '9', 'edit' => '1'))){
				...
				<form action="timetable.php" class="mb-lg validate" enctype="multipart/form-data" method="post" accept-charset="utf-8">
					...
					<div class="panel-body">
						<div class="row mt-sm">
							<div class="col-sm-4 col-sm-offset-2">
								<div class="form-group">
									...
									<select class="form-control" required disabled title="Must Be Required" data-plugin-selectTwo data-width="100%" data-minimum-results-for-search="Infinity" name="id_class" onchange="get_classsection(this.value)">
										<option value="">Select</option>';
										$sqllmscls	= $dblms->querylms("SELECT class_id, class_name 
																		FROM ".CLASSES."
																		WHERE class_id != '' AND class_status = '1' AND is_deleted != '1'
																		AND class_id = '".$value_edit['id_class']."'
							...
							<div class="col-sm-4">
								<div class="form-group" id="getclasssection">
									...
									<select class="form-control" disabled data-plugin-selectTwo data-width="100%" data-minimum-results-for-search="Infinity" id="id_section" name="id_section" required>
										<option value="">Select</option>';
						...
						<div class="form-group mt-sm">
							<label class="col-sm-1 col-sm-offset-2 control-label mt-lg">Status <span class="required">*</span></label>
							<div class="col-md-9">
							...
							foreach($daytypes as $day) {
								if($day['id'] < 7){
									$sqllmsperiods  = $dblms->querylms("SELECT p.period_id, p.period_name
																			...
																			ORDER BY p.period_ordering ASC");
														...
														<div class="col-sm-12">
															<div class="col-md-4 form-group">
																...
																<select data-plugin-selectTwo data-width="100%" name="id_subject['.$sri.']" id="id_subject['.$sri.']" title="Must Be Required" class="form-control populate">
																	<option value="">Select</option>
																	<option value="99999" '.($valuedetail['id_subject'] == '99999' ? 'selected' : '').'>Assembly</option>
															...
															<div class="form-group">
																<div class="col-md-12">
																	<label class="control-label">Lecture Time strt: </label>
																	<div class="input-timerange input-group">
																		<span class="input-group-addon">
																			<i class="fa fa-clock-o"></i>
																		</span>
																		<input type="text" class="form-control valid" name="start_time['.$sri.']" id="start_time" value="'.$valuedetail['start_time'].'" data-plugin-timepicker aria-required="true">
																		<span class="input-group-addon">to</span>
																		<input type="text" class="form-control" name = "end_time['.$sri.']" id="end_time" value="'.$valuedetail['end_time'].'" data-plugin-timepicker aria-required="true">
																	</div>
																</div>
															</div>
														</div>
