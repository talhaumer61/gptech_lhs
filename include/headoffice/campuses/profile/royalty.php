...
//-----------------------------------------------------
// Royalty Type
$sqllmsRoyalty	= $dblms->querylms("SELECT royalty_type
	...
	<form action="#" class="form-horizontal validate" enctype="multipart/form-data" method="post" accept-charset="utf-8">
		...
		<fieldset>
			<div class="panel-body">
				<div class="form-group mb-lg">
					<div class="col-md-12">
						<label class="control-label">Royalty Type <span class="required">*</span></label>
						<select data-plugin-selectTwo data-width="100%" name="royalty_type" id="royalty_type" required title="Must Be Required" class="form-control populate" onchange="get_royalty_type(this.value)">
							<option value="">Select</option>';
							foreach($rolyaltyType as $type){
								if($type['id']==3){
									echo'<option  value="'.$type['id'].'|'.$_GET['id'].'"'; if($valRoyaltyCheck['royalty_type'] == $type['id']){echo'selected';} echo' >'.$type['name'].'</option>';
								}
							}
							echo'
						</select>	
				...
				<div class="getroyaltytype">';	
					//Show According to Royalty Type
									...
									<tbody>';
										...
										while($valPart = mysqli_fetch_array($sqllmsParticulars)){
											...
											$sqllmsRoyalty	= $dblms->querylms("SELECT r.id, r.grand_total, d.detail_id, d.id_particular, d.id_class, d.no_of_std, d.amount_per_std, d.tuitionfee_percentage, d.total_amount,
																				...
																				AND r.royalty_type = '".$valRoyaltyCheck['royalty_type']."'
									...
									<tbody>';
										...
										while($valPart = mysqli_fetch_array($sqllmsParticulars)){
											...
											if($valPart['part_id'] == 3) {
												...
												// $totalAmount = $totalAmount + $valRoyalty['total_amount'];
											}
										}
										echo'
									</tbody>
					...
					}elseif($valRoyaltyCheck['royalty_type'] == 3){
						//--------------- Royalty Particulars ----------------
						$sqllmsParticulars = $dblms->querylms("SELECT part_id, part_name
																	FROM ".ROYALTY_PARTICULARS."
																	WHERE part_status = '1' AND is_deleted != '1'
		...
		<div class="panel-footer">
			<div class="row center">
				<button type="submit"  name="submit_royalty" id="submit_royalty" class="btn btn-primary">Update Royalty</button>
			</div>
		</div>
	</form>
</div>';
