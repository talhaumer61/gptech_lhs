                ...
                <div class="col-md-4 col-md-offset-2">
                    <label class="control-label">Campus <span class="required">*</span></label>
                        ...
                        <option value="">Select</option>';
                            $sqllms_camp	= $dblms->querylms("SELECT campus_id, campus_name 
                                                FROM ".CAMPUS."
                                                WHERE campus_status = '1' AND is_deleted != '1'
                                                ORDER BY campus_name ASC");
                            while($value_camp = mysqli_fetch_array($sqllms_camp)) 
                            {
                                if($value_camp['campus_id'] == $campus){
                                    echo '<option value="'.$value_camp['campus_id'].'" selected>'.$value_camp['campus_name'].'</option>';
                                }else{
							...
							<tr style="overflow: hidden;">
								...
								foreach($classes as $class) { 
									...
									$sqllmsdetail	= $dblms->querylms("SELECT c.class_name, d.dated, d.start_time, d.end_time, e.emply_name, s.subject_name, s.subject_code, r.room_no
																				...
																				AND d.dated = '".$rowsdet['dated']."' LIMIT 1
																			");
									//--------------------------------------
