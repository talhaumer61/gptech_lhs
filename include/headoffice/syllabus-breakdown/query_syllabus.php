...
if(isset($_POST['submit_syllabus'])) { 
	...
	if(mysqli_num_rows($sqllmscheck)) {
		...
		if(!empty($_FILES['syllabus_file']['name'])) { 
			...
			$originalImage	= $img_dir.to_seo_url(cleanvars($_POST['id_session'].'-'.$_POST['id_class'])).'_'.$syllabus_id.".".($extension);
			$img_fileName	= to_seo_url(cleanvars($_POST['id_session'].'-'.$_POST['id_class'])).'_'.$syllabus_id.".".($extension);
...
if(isset($_GET['deleteid'])){
	...
	if($sqllms) { 
		$remarks = 'Syllabus Deleted ID: "'.cleanvars($_GET['deleteid']).'" details';
			$sqllmslog  = $dblms->querylms("INSERT INTO ".LOGS." (
																  id_user 
																, filename 
																, action
