...
if(($_SESSION['userlogininfo']['LOGINTYPE']  == 1) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '61', 'edit' => '1'))){ 
	$sqllms	= $dblms->querylms("SELECT  syllabus_id, syllabus_status, syllabus_term, id_session, syllabus_file, id_month, id_class, id_subject, note
								   FROM ".SYLLABUS." 
					...
					<div class="col-md-9">
						<select class="form-control" required title="Must Be Required" data-plugin-selectTwo data-width="100%" data-minimum-results-for-search="Infinity" name="syllabus_term">
							...
							while($valTerms = mysqli_fetch_array($sqlTerms)) {
								echo '<option value="'.$valTerms['term_id'].'" '.($valTerms['term_id'] == $rowsvalues['syllabus_term'] ? 'selected' : '').'>'.$valTerms['term_name'].'</option>';
							}
							echo'
						</select>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-md-9">
						<select class="form-control" required title="Must Be Required" data-plugin-selectTwo data-width="100%" data-minimum-results-for-search="Infinity" id="id_month" name="id_month">
							<option value="">Select</option>';
							for($i = 1; $i<=3; $i++ ){
								echo '<option value="'.$i.'"'; if($rowsvalues['id_month'] == $i){ echo 'selected';} echo'>Assessment : '.$i.'</option>';
							}
							echo'
						</select>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-md-9">
						<div class="radio-custom radio-inline">
							<input type="radio" id="syllabus_status" name="syllabus_status" value="1" '.($rowsvalues['syllabus_status'] == 1 ? 'checked' : '').'>
							<label for="radioExample1">Active</label>
						</div>
						<div class="radio-custom radio-inline">
							<input type="radio" id="syllabus_status" name="syllabus_status" value="2" '.($rowsvalues['syllabus_status'] == 2 ? 'checked' : '').'>
							<label for="radioExample1">Active</label>
						</div>
					</div>
				</div>
